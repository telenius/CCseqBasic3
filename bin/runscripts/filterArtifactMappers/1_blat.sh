...
#CapturePipePath="/t1-home/molhaem2/telenius/CC2/norm/VS101"
...
OPTS=`getopt -o o:,f:,r:,p:,u:,e: --long genomefasta:,oligofile:,refile:,pipepath:,ucscbuild:,extend: -- "$@"`
if [ $? != 0 ]
then
    exit 1
fi
...
echo "blat -minScore=${blatminscore} -minIdentity=${blatminidentity} -maxIntron=${blatmaxintron} -tileSize=${blattilesize} ${genomefasta} ${file} ${basename}_blat.psl"
echo "blat -minScore=${blatminscore} -minIdentity=${blatminidentity} -maxIntron=${blatmaxintron} -tileSize=${blattilesize} ${genomefasta} ${file} ${basename}_blat.psl" >> "/dev/stderr"
blat -minScore=${blatminscore} -minIdentity=${blatminidentity} -maxIntron=${blatmaxintron} -tileSize=${blattilesize} ${genomefasta} ${file} ${basename}_blat.psl
...
# Preparing the input files for the dpn-fragment generation perl script
echo 
echo "Preparing the input files for the dpn-fragment generation perl script.."
echo "Preparing the input files for the dpn-fragment generation perl script.."  >> "/dev/stderr"
