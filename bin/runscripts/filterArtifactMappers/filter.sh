...
OPTS=`getopt -o p: --long parameterfile:,noploidyfilter,pipelinecall,extend: -- "$@"`
if [ $? != 0 ]
then
    exit 1
fi
...
#input_filename_path /hts/data2/jdavies/15_Methods_paper/00fqfiles/08_30genes_hiseeq/30ghC1_S6_REdig.sam
...
echo "dataprefix ${dataprefix}" >> parameters_norm.log
...
echo "${CaptureFilterPath}/1_blat.sh -o ${oligofile} -f ${GenomeFasta} -u ${ucscBuild} -r ${recoordinatefile} -p ${CaptureFilterPath} -e ${extend}"
echo "${CaptureFilterPath}/1_blat.sh -o ${oligofile} -f ${GenomeFasta} -u ${ucscBuild} -r ${recoordinatefile} -p ${CaptureFilterPath} -e ${extend}"  >> "/dev/stderr"

cp ${CaptureFilterPath}/1_blat.sh .
chmod u=rwx 1_blat.sh
./1_blat.sh -o ${oligofile} -f ${GenomeFasta} -u ${ucscBuild} -r ${recoordinatefile} -p ${CaptureFilterPath} -e ${extend}
rm -f 1_blat.sh
...
for file in TEMPdir2/*PF.gff
do
  
    oligoname=$( echo $file | sed 's/.*'${dataprefix}'_//' | sed 's/_noPF.gff$//' | sed 's/_PF.gff$//' )
    echo "${oligoname} .."
    newname=$( echo $file | sed 's/.*\///' | sed 's/.gff$//' )
...
#Combined_reads_REdig_CC2_Hba-1.sam
    ...
    samtools view -H -o TEMPheading.sam TEMP.bam

    # We check PLOIDY filter sam region count, and filter for ploidy..
    echo "${basename} - checking the need of filtering for PLOIDY regions.."
    ...
    echo "${basename} - checking the need of filtering for BLAT regions.."
    ...
    if [ -s "${outputfolder}/${dataprefix}_${basename}_forBlatAndPloidyFiltering.gff" ] ; then
        echo "${basename} - filtering for PLOIDY and BLAT regions.."
    ...
    printThis="After PLOIDY and BLAT filter we have ${samfragments} sam fragments left in our ${basename} reporter fragment file."
    printToLogFile
    ...
    printThis="Combining filtered reporter SAM file to the capture SAM for ${basename} .."
    printToLogFile    
    ...
    cat TEMP_sorted.sam >> TEMP_combined.sam
    ls -lht | grep TEMP >> "/dev/stderr"
...
ls -lht TEMPheading.sam
ls -lht TEMP_combined.sam

ls -lht TEMPheading.sam >> "/dev/stderr"
ls -lht TEMP_combined.sam >> "/dev/stderr"

cat TEMPheading.sam | sed 's/SO:coordinate/SO:unsorted/' | cat - TEMP_combined.sam > ${outputfolder}/filtered_combined.sam
rm -f TEMPheading.sam TEMP_combined.sam
...
echo
ls -lht ${outputfolder} | grep "_filtered.sam"
